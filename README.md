...
`Z2K_SILENCE_SECONDS` секунд (по умолчанию 5) ничего нового от сайта не
пришло, засчитывает стратегии провал. Штатные признаки при этом проверяются
первыми и никуда не деваются. Соединение сбрасывается клиенту (тот же
`reset`), чтобы он не досиживал свой таймаут, а переоткрыл сайт на следующей
стратегии.

Голый ACK ответом не считается: это и есть картина блокировки — запрос до
сайта дошёл, он его подтвердил, а ответ вырезан на обратном пути. Пришедшие
данные ожидание продлевают, но не отменяют: часть блокировок пропускает
начало ответа и режет поток дальше, и тогда «ответ был» означало бы «провала
не будет никогда». Ожидание снимается, только когда обмен состоялся: получено
больше 4 КБ (тот же порог, что у штатного признака удачи) или сайт сам закрыл
соединение.

Молчания считаются отдельным счётчиком домена, который гаснет только временем.
Штатный счётчик провалов обнуляет любой успех, и для сайта, часть запросов
которого отдаётся, а часть режется (обычное дело для крупных сайтов с их
поддоменами и узлами CDN), это означало бы «не переключать никогда»: замер —
восемь соединений, четыре молчания, четыре успеха, ни одной смены стратегии.
