...
`standard_success_detector`), своих у z2k нет. Параметры `circular` выставлены
по документации zapret2, а не «по умолчанию»:
...
На Keenetic это бывает часто — сборка больших ClientHello выключена
(`--reasm-disable=tls_client_hello`, иначе браузеры виснут), а ClientHello
Chrome и Firefox с постквантовым ключом не помещается в один сегмент; такие
рукопожатия получают фейк из снимка, как раньше. Замер на линии автора: 18 из
30 ClientHello за минуту были многосегментными.
