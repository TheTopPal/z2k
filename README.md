...
#### Поправки к штатному детектору

Штатный детектор bol-van считает провалом любую исходящую ретрансмиссию,
любой входящий RST и DPI-редирект. На живой линии этого мало в одну сторону и
слишком много в другую, поэтому z2k его не заменяет, а оборачивает: штатная
логика вызывается как есть, а поверх добавлены поправки, каждая из которых
получена замером на роутере.

- **Ретрансмиссия считается только на первом запросе.** Иначе провалом
  становится обычная потеря пакета в уже работающей сессии, и рабочая
  стратегия уезжает при 29 успехах против 3 провалов.
- **Фатальный TLS-алерт до ответа сайта — провал.** Сайт подтверждает
  приветствие, отвечает семибайтовым алертом и закрывается: ретрансмитить
  нечего, RST нет, штатный детектор слеп, стратегия не сменится никогда.
- **RST от самого сайта провалом не считается.** Отличаем по TTL:
  подделанный на пути приходит с TTL, который потоку настоящего сервера
  принадлежать не может. Без этого apple.com уезжал с рабочей стратегии на
  нерабочую.
- **Живой сайт не ротируем.** Если в том же окне к тому же сайту прошли
  нормальные ответы, поддельный RST на первом байте провалом не считается.
- **Провал засчитывается той стратегии, на которой соединение началось.**
  Страница открывает десятки соединений разом; без этого замер 19.08 дал
  двенадцать смен стратегии за одну секунду.
- **QUIC — отдельный детектор по молчанию.** Штатный для UDP не работает в
  принципе: мёртвый QUIC-поток шлёт меньше пакетов, чем живой (браузер не
  повторяет Initial, а уходит на TCP). Замер по 1646 потокам: ни один порог
  эти классы не разделяет, различает их только время.

Эти поправки снимались 10.09.2026 и возвращены 11.09.2026.
