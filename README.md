...
Детекторы — штатные, из nfqws2 (`standard_failure_detector` /
`standard_success_detector`), плюс один свой — «сервер молчит» (см. ниже).
Параметры `circular` выставлены по документации zapret2, а не «по умолчанию»:

- **Неудача (TCP)** — три ретрансмиссии первого запроса в пределах 32 КБ
  исходящих (`retrans=3`, `maxseq=32768`): при тихом дропе ClientHello клиент
  сам повторяет его с задержками 1-2-4 с, три повтора набираются за ~7 с. Также
  входящий RST или HTTP-редирект на чужой домен в первых 4 КБ ответа
  (`inseq=4096`).
- **Удача (TCP)** — сервер прислал больше 4 КБ или клиент отправил больше 32 КБ
  без застревания: DPI так себя не ведёт. Удача сбрасывает счётчик неудач.
- **Неудача/удача (UDP)** — «отослано ≥ 5, принято ≤ 1» / «принято ≥ 2»
  (`udp_out=5`, `udp_in=1`; `udp_out=5` вместо документированных 4 — по замеру
  на 1646 QUIC-потоках, где порог по исходящим классы не разделяет).
- **RST после неудачи** (`reset`) — зафиксировав провал, nfqws2 сбрасывает
  соединение, чтобы клиент не ждал таймаута, а сразу переоткрыл его на
  следующей стратегии. Выключается `Z2K_CIRCULAR_RESET=0` в конфиге.
- Счётчик: три неудачи с зазором не больше 60 с (`fails=3`, `time=60`); на пулах
  видео зазор 300 с — по замеру на телевизорах, где попытки редкие.

Состояние ротации ведётся отдельно для IPv4 и IPv6 одного хоста
(`family_split`, наш патч) — провалы по v6 не уводят рабочую стратегию v4.

Обрыв на 16 КБ в ротации не участвует — у него своя система, см. выше.

#### Молчание сервера — тоже провал

Все три штатных признака активные: ретрансмиссия, RST, редирект. Самый частый
сегодня блок не даёт ни одного: DPI пропускает приветствие до сайта, сайт
подтверждает его целиком и замолкает — ответ вырезан на обратном пути.
Ретрансмитить клиенту нечего, RST не приходит, и ротация стоит на первой
стратегии вечно; в браузере это «ERR_TIMED_OUT».

Поэтому z2k ставит таймер на каждое первое обращение и, если за
`Z2K_SILENCE_SECONDS` секунд (по умолчанию 5) не пришло ни байта ответа,
засчитывает стратегии провал. Штатные признаки при этом проверяются первыми и
никуда не деваются. Ответом считаются только данные: голый ACK — это и есть
картина блокировки. Соединение при этом сбрасывается клиенту (тот же `reset`),
чтобы он не досиживал свой таймаут, а переоткрыл сайт на следующей стратегии.

Провал засчитывается той стратегии, на которой соединение началось: страница
открывает десятки соединений разом, и без этого одно молчание пролистало бы
весь пул. Закреплённую в панели стратегию детектор не трогает, на UDP-пулах не
работает. Выключается `Z2K_SILENCE_DETECT=0` в `/opt/zapret2/config`.
...
### Отдельный профиль под хост: `lists/custom-profiles/`

`custom-strategies` **переопределяет** существующий пул. Если же одному хосту нужна стратегия, отличная от соседей по тому же пулу, нужен **отдельный профиль** — это `lists/custom-profiles/`.

Каждый файл `*.conf` — самостоятельный профиль nfqws2: строка опций со своим `--hostlist-domains` (или `--hostlist`), `--filter-*` и фиксированным `--lua-desync`. Можно в несколько строк, `#` и пустые строки игнорируются. Профили вставляются **перед** категорийными, а nfqws2 применяет **первый совпавший** — поэтому такой профиль перехватывает свои хосты раньше общего пула:

```
/opt/zapret2/lists/custom-profiles/discord-updates.conf
```

Классический случай — `updates.discord.com`: апдейтер Discord на Rust/rustls шлёт другое приветствие, чем сам сайт в браузере, и требует иной стратегии, но в пуле `rkn` у них общий ключ ротации. Отдельный профиль разносит их. Готовый образец — `docs/examples/custom-profiles/discord-updates.conf.example`.

Полный обход Discord через z2k проверен на живом роутере и требует **двух** профилей с одной и той же rustls-стратегией: `updates.discord.com` (манифест обновлений) и `dl.discordapp.net,dl2.discordapp.net` (загрузка модулей, отдельный CDN, тот же rustls-фингерпринт). С ними свежая установка Discord целиком — манифест, ~100 МБ модулей и голос — проходит напрямую.

Стратегию записывайте **буквально** (сами опции), а не номером из арсенала: номера тасуются при обновлении, а профиль обязан его пережить. Как и `custom-strategies`, файлы читаются при каждой пересборке — переживают обновление и переустановку. Гнилой файл роняет генерацию fail-closed, рабочий конфиг при этом цел. Та же осторожность: ошибка в строке валит весь обход, проверяйте кандидата движком до применения.
...
│   │   ├── z2k-silence.lua         # детектор «сервер молчит» для ротации
