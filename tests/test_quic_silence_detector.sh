#!/bin/sh
# tests/test_quic_silence_detector.sh — обёртка над lua-харнесом
# tests/test_quic_silence_detector.lua, чтобы общий прогон (tests/run_all.sh) и CI
...
if [ -z "$LUA" ]; then
    printf "[PASS] quic_silence_detector: пропущено (lua локально не установлена)\n"
    exit 0
fi

cd "$PROJECT_ROOT"
exec "$LUA" tests/test_quic_silence_detector.lua
