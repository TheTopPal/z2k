...
-- Часы движка: тесты двигают их вручную, чтобы проверить окно счётчика.
FAKE_NOW = 1700000000
...
-- ----- 11. чужой успех не гасит молчания домена -----------------------------
--
-- Замер на стенде 11.09.2026: домен, где половина соединений отдаётся, а
-- половина режется, не ротируется НИКОГДА. Штатный счётчик провалов хоста
-- обнуляется любым успехом (automate_failure_counter_reset), и до порога он
-- не доходит: 8 соединений — 4 провала, 4 сброса, 0 ротаций. Для крупного
-- сайта это норма жизни: мелочь отдаётся, основной ресурс режется, человек
-- видит «полуработает».
--
-- Поэтому молчания считаются ОТДЕЛЬНО, своим счётчиком в записи хоста: он
-- гаснет временем (то же окно time), а не чужим успехом.
do
    reset_state()
    COUNTER_VERDICT = false          -- штатный счётчик до порога не доходит
    for i = 1, 3 do
        local crec = {}
        arm(crec)
        fire_one()
    ...
    -- Одиночное молчание при живом домене стратегию не трогает.
    reset_state()
    COUNTER_VERDICT = false
    local crec = {}
    arm(crec)
    fire_one()
    is("одного молчания мало", 1, HREC.nstrategy)

    -- Счётчик молчаний живёт окном time: редкие молчания не накапливаются
    -- бесконечно и не ротируют рабочий домен раз в сутки.
    reset_state()
    COUNTER_VERDICT = false
    for i = 1, 2 do
        local crec = {}
    ...
    FAKE_NOW = FAKE_NOW + 120        -- прошло больше окна (time=60)
    crec = {}
    arm(crec)
    fire_one()
